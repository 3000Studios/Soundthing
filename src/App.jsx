  ...
  const stopRecording = () => {
    if (mediaRecorderRef.current && isRecording) {
      ...
      setTimeout(() => {
        ...
        a.href = url
        a.download = `soundthing_${new Date().toISOString().slice(0,19)}.webm`
        a.click()
      }, 100)
    }
  }
  ...
  return (
    <div style={{ maxWidth: 1200, margin: '0 auto', padding: 16 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        <h1 style={{ margin: 0, fontSize: 24 }}>🎙️ Soundthing RF Detector</h1>
        <DataExplanation />
      </div>

      <div style={{ display: 'flex', gap: 8, marginBottom: 16, background: 'rgba(255,255,255,0.05)', padding: 4, borderRadius: 8 }}>
        {tabs.map(tab => {
          const Icon = tab.icon
          return (
            <button
      ...
      {activeTab === 'audio' && (
        <AudioTab
          ...
          setMicrophoneGain={setMicrophoneGain}
          outputVolume={outputVolume}
          setOutputVolume={setOutputVolume}
          ultrasonicMode={ultrasonicMode}
          setUltrasonicMode={setUltrasonicMode}
      ...
      <div style={{ 
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button
            ...
            style={{
              padding: '10px 20px',
              background: isDetecting ? '#dc2626' : '#10b981',
              border: 'none',
              borderRadius: 6,
              color: '#fff',
              cursor: 'pointer',
          ...
          >
            {isDetecting ? <><Pause size={18} /> Stop</> : <><Play size={18} /> Start</>}
          </button>
          <button
          ...
          >
            {isRecording ? <><Square size={18} /> Stop Rec</> : <><Circle size={18} /> Record</>}
          </button>
        </div>
        <div style={{ fontSize: 14, color: '#9ca3af' }}>
          {isDetecting ? '🟢 Active' : '⚫ Idle'} • {isRecording ? `⏺️ ${Math.floor(recordingTime / 60)}:${String(recordingTime % 60).padStart(2, '0')}` : 'Not recording'}
        </div>
      </div>
    </div>
  )
